...
const validations =Yup.object().shape({
    ...
    descripcion: Yup.string().min(2,'Descripción muy corto').max(50,'Descripción muy largo').required('Obligatorio')
})

export default function Formulario({route,navigation}){
    const {status} = route.params;
    const {ropa,lista,setRopa,setLista}= useContext(AlumnosContext);

    return(
        <View style={styles.container}>
            <Text style={styles.header}>Ropa</Text>

            <Formik
                initialValues={ropa}
                onSubmit={(values,{resetForm})=>{
                     firebase.database().ref('Ropa/'+ropa.id).update(ropa).then(()=>{
                         alert("Enviado")
                     })
                    const temporal = lista.filter(al=>al.id!=ropa.id);//!==
                    //alert('enviado')
                ...
                ({handleChange, handleBlur, handleSubmit, setFieldValue, handleReset, errors, values})=>(
                    <View>
                        ...
                        <View style={styles.picker}>
                            ...
                            >
                                <Picker.Item color="grey" label="Talla" value="" />
                                <Picker.Item color="black" label="S" value="S"/>
                                <Picker.Item color="black" label="M" value="M"/>
                                <Picker.Item color="black" label="L" value="L"/>
                            </Picker>
                        </View>
